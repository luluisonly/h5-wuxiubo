...
$(function() {
	...
	$('.index .pointer').click(function(){
		...
		var time1 = setTimeout(function(){
			//此链接应该更换成正式上线的链接
			window.location.href = "http://mktm.10101111.com/html5/2016/szzc/form.html?WT.mc_mk=201401357&tdaz=hEtE5c";
		},200);
	});
});
//canvas绘画
	...
	if(p == 1){
		...
		var loadTime = setInterval(function(){
			if(num == 100) {
				...
				//解决苹果黑一下的问题
				if(isAndroid || isIphone4){
					$('.index .startBtn').css({'background':'url(../src/pic_start.jpg)no-repeat','background-size':'100% 100%'});
				} else {
					console.log("ios终端");
					setCan();
					$('.canvas').show();
